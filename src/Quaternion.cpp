...
namespace reef_msgs{
  ...
  auto Quaternion::rand() -> Quaternion {
    Quaternion instRand(((double) std::rand() / RAND_MAX),((double) std::rand() / RAND_MAX),((double) std::rand() / RAND_MAX),((double) std::rand() / RAND_MAX));
    instRand.normalize();
    return instRand;
  }

auto Quaternion::quaternionMultiplication(Eigen::Matrix<double,4,1> p,Eigen::Matrix<double,4,1> q) -> Eigen::Matrix<double,4,1> {
  double pw = p(3,0);
...
auto Quaternion::toRotation() -> Eigen::Matrix3d {
  ...
  Quaternion::Quaternion(const double &_x, const double &_y, const double &_z, const double &_w):AngleRepresentationInterface(){
